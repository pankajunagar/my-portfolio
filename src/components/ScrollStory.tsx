...
function StoryText({ story, index, total, progress }: StoryProps) {
  ...
  return (
    <motion.div
      ...
      className={`absolute inset-0 flex flex-col justify-center transition-all duration-300 ${index === 0 ? "relative" : ""}`}
    >
      ...
      <p className="text-lg text-white/60 max-w-lg leading-relaxed mb-10">{story.description}</p>
...
function StoryVisual({ story, index, total, progress }: StoryProps) {
  const scale = useTransform(progress, [index / total, (index + 0.5) / total], [0.9, 1]);
  const opacity = useTransform(progress, [(index - 0.15) / total, index / total, (index + 0.8) / total, (index + 1) / total], [0, 1, 1, 0]);
  const rotate = useTransform(progress, [index / total, (index + 1) / total], [8, -8]);
    ...
    >
      <div className="relative w-full max-w-xl aspect-[16/11] group">
        ...
        <div className="relative glass-card p-1 rounded-[40px] border-white/20 shadow-[0_0_100px_rgba(3,142,199,0.2)] overflow-hidden h-full">
          <Image src={story.image} alt={story.title} fill className="object-cover rounded-[38px] opacity-90 brightness-110" />
          ...
          <div className="grid lg:grid-cols-2 gap-0 lg:gap-20 items-center">
            <div className="relative z-10 space-y-24 py-20 min-h-[400px]">
              {stories.map((story, index) => (
                <StoryText key={story.id} story={story} index={index} total={stories.length} progress={smoothProgress} />
              ))}
            </div>
            <div className="relative h-[60vh] lg:h-[80vh] flex items-center justify-center">
              {stories.map((story, index) => (
                <StoryVisual key={story.id} story={story} index={index} total={stories.length} progress={smoothProgress} />
              ))}
            </div>
          </div>
        </div>
          ...
          {stories.map((story) => (
            <div key={story.id} className="space-y-10">
                ...
                <div className="relative glass-card p-1 rounded-3xl border-white/10 shadow-2xl overflow-hidden h-full min-h-[250px] bg-white/[0.02]">
                  <div className="relative w-full h-full overflow-hidden rounded-[22px]">
                    <div className="w-full h-full">
                      <Image src={story.image} alt={story.title} fill className="object-cover opacity-90 brightness-110" />
                    </div>
                  </div>
                </div>
              </motion.div>
              ...
              >
                ...
                <p className="text-white/60 leading-relaxed max-w-sm mx-auto">{story.description}</p>
                <div className="flex flex-wrap justify-center gap-3">
                  {story.features.map((f) => (
                    <span key={f} className="px-4 py-1.5 rounded-xl bg-white/5 border border-white/10 text-[10px] font-bold text-white/70">
                      {f}
                    </span>
                  ))}
                </div>
